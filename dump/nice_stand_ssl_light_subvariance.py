...
class IE():
    ...
    def feature_decompose(self, runId):
        ...
        cpm_sub_emb_encoder = CheckpointManager(prefix="SubEmbEnc",base_dir=f"/home/jbhol/EEG/gits/BrainCoder/model/{runId}")
        ...
        self.optimizer = torch.optim.Adam(itertools.chain(self.Enc_eeg.parameters(),
                                                            self.sub_emb_encoder.parameters(),
        ...
        for sub_i in range(2,3):
            ...
            for e in range(self.pretrain_img_epoch):
                ...
                for i, (data1,data2,data3) in enumerate(self.dataloader):
                    ...
                    # Adversrial 
                    Discj1  = self.subject_discriminator(E_cj1_cls_embeddings, alpha)
                    Disck1  = self.subject_discriminator(E_ck1_cls_embeddings, alpha)
                    batch_size = eeg.size(0)
                    s1 = torch.ones(batch_size).cuda().type(self.LongTensor)  # Subject labels
                    ...
                    # diff_sub_embeddings = torch.abs(E_cj1_sub_embeddings - E_ck1_sub_embeddings)
                    # j_sub_emb = E_cj1_sub_embeddings - diff_sub_embeddings
                    # k_sub_emb = E_ck1_sub_embeddings - diff_sub_embeddings

                    # remove subject variance
                    # E_cj1_cls_embeddings = E_cj1_cls_embeddings - j_sub_emb
                    # E_ck1_cls_embeddings = E_ck1_cls_embeddings - k_sub_emb

                    # reconstruction
                    # E_cj1_decoded = self.sub_emb_decoder(E_cj1_cls_embeddings,[])
                    # E_ck1_decoded = self.sub_emb_decoder(E_ck1_cls_embeddings,[])

                    # # seperate subject variance from main embeddings
                    # sub_emb_loss_j = 1 - F.cosine_similarity(E_cj1_sub_embeddings,j_sub_emb).mean()
                    # sub_emb_loss_k = 1 - F.cosine_similarity(E_ck1_sub_embeddings,k_sub_emb).mean()
                    # sub_emb_loss  = (sub_emb_loss_j + sub_emb_loss_k)/2
                    ...
                    # norm features for loss calculation
                    E_cj1_cls_embeddings = self.norm_feat(E_cj1_cls_embeddings)
                    # E_cj1_sub_embeddings = self.norm_feat(E_cj1_sub_embeddings)
                    
                    E_ck1_cls_embeddings = self.norm_feat(E_ck1_cls_embeddings)
                    # E_ck1_sub_embeddings = self.norm_feat(E_ck1_sub_embeddings)

                    # E_cj1_backbone, E_ck1_backbone = self.norm_feat(E_cj1_backbone), self.norm_feat(E_ck1_backbone)
                    # E_cj1_decoded, E_ck1_decoded = self.norm_feat(E_cj1_decoded), self.norm_feat(E_ck1_decoded)

                    # img loss
                    img_loss = self.get_contrastive_loss(E_cj1_cls_embeddings, image_features, labels)
                    img_val_loss = self.get_contrastive_loss(E_ck1_cls_embeddings, image_features_2, labels) # This subject is not trained explicitly
                    
                    # recon from subjec emb + cls_emb
                    # recon_loss_j = self.criterion_l2(E_cj1_backbone, E_cj1_decoded)
                    # recon_loss_k = self.criterion_l2(E_ck1_backbone, E_ck1_decoded)
                    # recon_loss = (recon_loss_j + recon_loss_k)/2

                    # F.cosine_similarity(x2_backbone,x_backbone).mean()
                    # F.cosine_similarity(x2_variance,x_variance).mean() # Subject specific variance similarity :  0.67  
                    # F.cosine_similarity(x2,x).mean() # 0.04
                    # F.cosine_similarity(x_comm,x2_comm).mean() # 1.0  # when subject variance is removed.
                    # F.cosine_similarity(x_comm_proj,x2_comm_proj).mean() # 0.90   


                    lambda_domain = 0.2
                    adversrial_loss = lambda_domain * subject_loss

                    # increase sub_emb_loss
                    loss = img_loss + adversrial_loss
                ...
                if (e + 1) % 1 == 0:
                    ...
                    # ' recon_loss: %.4f' % recon_loss.detach().cpu().numpy(),  
                    # ' sub_emb_loss: %.4f' % sub_emb_loss.detach().cpu().numpy(),  
                    ' adversrial_loss: %.4f' % adversrial_loss.detach().cpu().numpy(),  
                    ' alpha: %.4f' % alpha,  
                    )
