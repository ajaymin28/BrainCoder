        ...
        for sub_i in range(2,3):
            ...
            for e in range(self.pretrain_img_epoch):
                ...
                for i, (data1,data2,data3) in enumerate(self.dataloader):
                    ...
                    # Encourage E_c and Ej/Ek to be orthogonal  E_c.Ek = 0
                    dot_product_j = torch.sum(E_cj1_cls_embeddings * E_cj1_sub_embeddings, dim=1)  # Batch-wise dot product
                    ortho_loss_j =  torch.mean(dot_product_j.pow(2))
                    dot_product_k = torch.sum(E_ck1_cls_embeddings * E_ck1_sub_embeddings, dim=1)  # Batch-wise dot product
                    ortho_loss_k =  torch.mean(dot_product_k.pow(2))
                    ortho_loss = (ortho_loss_j+ortho_loss_k)/2


                    # Encourage E_j and Ek to be orthogonal as well
                    dot_product = torch.sum(E_ck1_sub_embeddings * E_cj1_sub_embeddings, dim=1)  # Batch-wise dot product
                    sub_ortho_loss =  torch.mean(dot_product.pow(2))


                    labels = torch.arange(eeg.shape[0])  # used for the loss
                    labels = Variable(labels.cuda().type(self.LongTensor))

                    # Encorage E_c to be same as image features.
                    ...
                    # Encuorage CLS embeddings to be close
                    ec_emb_loss = self.criterion_l2(E_cj1_cls_embeddings,E_ck1_cls_embeddings)

                    # increase sub_emb_loss
                    loss = ec_emb_loss + img_loss + sub_ortho_loss + ortho_loss
                ...
                if (e + 1) % 1 == 0:
                    ...
                    ' ortho_loss: %.4f' % ortho_loss.detach().cpu().numpy(),  
                    ' sub_ortho_loss: %.4f' % sub_ortho_loss.detach().cpu().numpy(),  
                    ' ec_emb_loss: %.4f' % ec_emb_loss.detach().cpu().numpy(),  
                    ' alpha: %.4f' % alpha,  
                    )
