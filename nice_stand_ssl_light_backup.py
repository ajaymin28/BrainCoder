        ...
        for sub_i in range(2,5):
            ...
            for e in range(self.pretrain_epoch):
                ...
                for i, (data1,data2,data3) in enumerate(self.dataloader):
                    ...
                    # obtain the features
                    Lj = self.Proj_eeg(self.Enc_eeg(eegMean))
                    Lk = self.Proj_eeg(self.Enc_eeg(eegMean2))
                    
                    # SEj_pred = self.eeg_subj_emb(Lj)
                    # SEk_pred = self.eeg_subj_emb(Lk)


                    # find difference
                    differences = torch.abs(Lj - Lk)
                    ...
                    # common_features = softmax_weights * Lj
                    # common_loss = torch.sum((common_features - Lj) ** 2, dim=1).mean()

                    # different_features = softmax_weights * differences

                    # REMOVE COMMON FEAT to get sub embeddings
                    # SEj_cal = Lj - common_features
                    # SEk_cal = Lk - common_features

                    # SE loss
                    # SE_loss = self.criterion_l2(Lj,SEj_cal)
                    # SE_loss += self.criterion_l2(Lk,SEk_cal)
                    
                    Lj_common_features = softmax_weights * Lj
                    Lk_common_features = softmax_weights * Lk


                    # common features Encoder loss
                    # E_loss = self.criterion_l1(Lj,Lj_common_features)
                    # E_loss += self.criterion_l1(Lk,Lk_common_features)

                    # IMG_LOSS = self.criterion_l1(Lj, image_features)
                    # IMG_LOSS += self.criterion_l1(Lk, image_features_2)

                    E_loss = self.get_contrastive_loss(feat1=Lj,feat2=Lj_common_features,labels=labels)
                    E_loss += self.get_contrastive_loss(feat1=Lk,feat2=Lk_common_features,labels=labels)

                    IMG_LOSS = self.get_contrastive_loss(feat1=Lj, feat2=image_features,labels=labels)
                    IMG_LOSS += self.get_contrastive_loss(feat1=Lk, feat2=image_features_2,labels=labels)
                    loss = (E_loss + 0.5*IMG_LOSS)/2

                    # loss, common_loss, difference_loss = self.commdiffloss(SEj,SEk)
                    ...
                    # labels1 = torch.ones(eegMean.shape[0])  # used for the loss
                    # labels1 = Variable(labels1.cuda().type(self.LongTensor))
                    # labels1 = labels1 * self.args.num_sub 


                    # labels_subi = torch.ones(eegMean.shape[0])  # used for the loss
                    # labels_subi = Variable(labels_subi.cuda().type(self.LongTensor))
                    # labels_subi = labels_subi * sub_i


                    # loss = self.criterion_cls(SEk,labels1)
                    # loss += self.criterion_cls(SEj,labels_subi)
                    ...
                    # loss += self.get_contrastive_loss(feat1=Ej,feat2=Ek,labels=labels)
                ...
                if (e + 1) % 1 == 0:
                    ...
                    print('Sub id', self.nSub, 
                        ...
                        #   '  common loss: %.4f' % common_loss.detach().cpu().numpy(),
                        #   '  diff loss: %.4f' %  difference_loss.detach().cpu().numpy()
                        )
